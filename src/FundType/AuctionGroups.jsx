...
import { getAuctionGroups } from "../api/api";
  ...
  <div
    ...
    <div className="flex items-start justify-between">
      ...
      <span>
        <ReactCountryFlag
          svg
          countryCode={currencyMeta[group.currency].flag}
          style={{ fontSize: "2em" }}
        />
      </span>
    </div>
    ...
    <div className="bg-yellow-400 rounded-xl px-4 py-3 flex justify-between font-semibold">
      ...
      <span>{formatCurrency(group?.currency, group?.totalFundAmount)}</span>
    </div>
  </div>
);
...
export default function AuctionOverview() {
  ...
  const fetchAuctionGroup = useCallback(async () => {
    setLoading(true);
    try {
      const response = await getAuctionGroups();
  ...
  return (
    <div className="p-4 md:p-8 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
        <div>
          <h1 className="text-2xl font-semibold">Auction Overview</h1>
          <p className="text-gray-500">Monitor all auction-based chit groups</p>
        </div>
      </div>
